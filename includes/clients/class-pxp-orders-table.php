 ...
 */
class Client_Order_List_Table extends WP_List_Table {
	...
	function column_order_date($item){		
        ...
        return sprintf('<a href="%1$s%2$s"><strong>%3$s</strong></a> %4$s',
			admin_url() . "admin.php?page=pxp-client-orders&order=",
			$item['ID'], 
            $item['order_date'],
    ...
    function prepare_items() {
         ...
         */
        $per_page = $this->get_items_per_page('client_orders_per_page', 10);
	...
	public function set_item_list($list)
	{
		$this->item_list = $list;
	}
